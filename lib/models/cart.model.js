...
schema.statics.modify = async function modify (cart) {
  const cartToUpdate = await this.findOne({ _id : cart._id });
  await cartToUpdate.set(cart);
...
schema.methods.setProductQuantity = async function setProductQuantity (product, quantity) {
  const productId = product._id || product;
  const line = this.content.find(line => line.product._id.toString() === productId);
