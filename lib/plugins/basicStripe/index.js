...
const basicShipping = require('../basicShipping');
  ...
  stepHandlers : [
    {
      ...
      async middleware (req, res, next) {
        ...
        if (req.method === `POST` && req.body.plugin_action === `validate_payment`) {
          ...
          return req.shop.cartManager.checkQuantities()
          .then(errors => {
            if (errors.length > 0) {
              return Promise.reject('Some products in the cart are not available anymore');
            }
          })
          .then(() => req.shop.cartManager.getTotal())
          ...
          .then(charge => {
            return req.shop.cartManager.fulfillStep(this.step, { handler : this.name, charge })
          })
          .then(() => {
            next();
            return;
        ...
        const email = await req.shop.cartManager.getEmail();
        return req.shop.cartManager.getTotal()
        .then(total => {
          res.render(path.join(__dirname, `payment.ejs`), {
            keyPublishable: config.keyPublishable,
            total : total * 100,
            csrf : req.csrfToken(),
            cart : req.shop.cartManager.cart,
            email,
          });
          return null;
        })
